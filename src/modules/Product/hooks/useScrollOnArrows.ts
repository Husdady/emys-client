...
/**
 * Hook for implements the scroll logic on Arrows
...
export default function useScrollOnArrows({
  ...
  const handleScroll = useCallback(() => {
    ...
    const scrollLeft = Math.ceil(productItemsRef.current?.scrollLeft ?? 0)
  ...
  }, [isLoading, handleScroll, productItemsRef.current])
